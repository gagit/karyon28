...
/**
 * Class DoctrineAbmGenerator
 *
 * @author gabriel
 */
class DoctrineAbmGenerator extends Generator
{
            ...
            $forceOverwrite)
    {
        ...
        //------------------------
        $dirRepository = sprintf('%s/Repository/', $this->bundle->getPath());
        $pathClassRepository = sprintf('%s/Repository/%sRepository.php', $this->bundle->getPath(), str_replace('\\', '/', $this->entity));

        if (!file_exists($dirRepository)) {
            $this->filesystem->mkdir($dirRepository, 0777);
        }
        if (!file_exists($pathClassRepository)) {
            //$this->filesystem->mkdir($pathClassRepository, 0777);
    ...
    /**
     * Genera la clase Repository.
     *
     * @param string $dirRepository Es el path a la carpeta donde se guardan los Repositorys en el Bundle
     */
    protected function generarClaseRepository($dirRepository)
    { $parts = explode('\\', $this->entity);
        array_pop($parts);
        
        $this->renderFile('repository/Repository.php.twig', $dirRepository, array(
            'bundle'            => $this->bundle->getName(),
             'namespace'        =>  $this->bundle->getNamespace(),
